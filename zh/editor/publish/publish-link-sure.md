...
![images](publish-link-sure/options.png)
...
- **资源服务器地址**
  ...
  - 若 **不填写** 该项，则发布包目录下的 `build/link-sure/remote` 文件夹将会被打包到构建出来的 cpk 包中。
...
构建完成后点击 **发布路径** 后面的 **打开** 按钮来打开构建发布包，可以看到在默认发布路径 build 目录下生成了 **link-sure** 目录，该目录就是导出的 **link-sure** 工程目录，cpk 包在 `build/link-sure/dist` 目录下。
